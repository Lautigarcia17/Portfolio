    ...
    <>
      <div className={styles.card} onClick={handleClick}>
        <div className={styles.imageContainer}>
          ...
          {project.status !== '' && <div className={styles.overlayText}>{language === 'es' ? project.status : project.statusEn}</div>}

        </div>
        <div className={styles.content}>
          <h2 className={styles.title}>{project.title}</h2>
          <p className={styles.text}>{language === 'es' ? project.text : project.textEn}</p>
          ...
          <div className={styles.buttonContainer}>
            ...
            {project.video ? (
              <button className={styles.button} onClick={(e) => { e.stopPropagation(); handleClick() }}>
                ...
                {translations.projects.viewVideo}
              </button>
            ) : (project.page !== '' &&
              <button className={styles.button} onClick={(e) => { e.stopPropagation(); handleClick() }}>
